...
def equal(u, v):
    ...
    assert u.D == v.D
    return {a: u.f[a] for a in u.f if u.f[a] != 0} == {a: v.f[a] for a in v.f if v.f[a] != 0}


def add(u, v):
    """Returns the sum of the two vectors"""
    assert u.D == v.D
    s = {k: u[k] + v[k] for k in set(u.f.keys()) | set(v.f.keys())}
    return Vec(u.D, s)


def dot(u, v):
    """Returns the dot product of the two vectors"""
    return sum([u[k] * v[k] for k in set(u.f.keys()) | set(v.f.keys())])
...
class Vec:
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, Vec):
            ...
            return other * self   #  Will cause other.__rmul__(self) to be invoked
